...
class Person:
  def __init__(self, name="trent", age=21):
    self.name = name
    self.age = age
  def printer(self):
      print(self.name)
...
def click():
    ...
    return messagebox.showinfo(title = 'User Info', message = 'Name: ' + enteredText1 +'\n'*2 + 'Password: ' + enteredText2), \
           textBox.insert('1.0','Thank you: '+enteredText1)


window = tk.Tk()
canvas = tk.Canvas(window,height=300,width=300,bg='grey',bd=10,relief='sunken')
canvas.grid(columnspan=300,rowspan=300)
#window.columnconfigure(0, minsize=300)
#\window.rowconfigure([0, 1], minsize=0)
...
label1 = tk.Label(window,
    ...
    height = 1,
    font=('Arial',9))
label1.grid(row=0,column=0)#,sticky='NSEW')  #,sticky='N')

entry1 = tk.Entry(window, width=25, bg='lightgreen')
entry1.grid(row=2, column=0) #grid makes the screen a grid #, sticky='N') can place the graphic on the N,S,W,E side of the window
...
entry2.grid(row=4, column=0,rowspan=True) #, sticky='N'))

button = tk.Button(window,text='Submit',width=5,command=click,font=('Arial',9))
...
textBox = tk.Text(window,width=19,height=10,bg='lightgreen')
textBox.grid(row=10,column=0)
window.mainloop()


###################################
def myTest(name="Trent"):
    alpha = {'name': name}
    print(name)
    return alpha
alpha=myTest(name="Bob")
alpha['name'] = 'Tom'
print(alpha)
#####################################
